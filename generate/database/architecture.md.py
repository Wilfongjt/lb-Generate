...
from lib.architecture_document import ArchitectureDocument
...
def main():
    ...
    print('  - target [repo]     : {}'.format(targetDev.getFolder('repo')))
    ...
    architectureDocument = ArchitectureDocument(apiConfiguration,
                                                folder=targetDev.getFolder('repo'),
    ...
    for apiName in apiNameList:
        ...
        # [Table Line]
        tableList = ['Table']
        tableList.extend(['{}'.format( getTable(database,m)) for m in apiConfiguration.getMethods(apiName,'parameters') ])
